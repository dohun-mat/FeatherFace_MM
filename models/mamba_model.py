...
# 주의: 사용자가 제공한 create_block 함수와 관련 의존성들이 import 되어 있어야 합니다.
# from vim.models_mamba import VisionRotaryEmbeddingFast, DropPath, RMSNorm, layer_norm_fn, rms_norm_fn
# from mamba_ssm import Mamba

class MambaFusion(nn.Module):
    def __init__(self, 
                 dim, 
                 depth=1,  # BiFPN 노드 하나당 Mamba Block 깊이 (기본 1 추천)
                 d_state=16,
                 if_bidirectional=True, # 양방향 스캔 사용 여부 (강력 추천)
                 if_rope=True,          # 위치 인코딩 사용 여부
                 patch_size=1,          # BiFPN 피처맵은 이미 패치화된 상태로 간주 (1로 설정)
                 drop_path_rate=0.0,
                 device=None,
                 dtype=None):
        super().__init__()
        ...
        self.if_rope_residual = False # 복잡도 낮추기 위해 False (원본 코드 참조)
        self.fused_add_norm = True    # 원본 코드 참조
        self.residual_in_fp32 = True
        
        # --- 1. 레이어 개수 설정 ---
        # 양방향 모드일 경우: 정방향(Forward) + 역방향(Backward) 쌍이 필요하므로 2배 생성
        num_layers = depth * 2 if if_bidirectional else depth
        
        # --- 2. Mamba Block 생성 (제공해주신 create_block 활용) ---
        # drop_path decay rule
        ...
        self.layers = nn.ModuleList([
            create_block(
                ...
                ssm_cfg=None,
                norm_epsilon=1e-5,
                rms_norm=True, # Vim 기본값
                residual_in_fp32=self.residual_in_fp32,
                fused_add_norm=self.fused_add_norm,
                layer_idx=i,
                if_bimamba=False, # 구조적 양방향을 쓰므로 개별 블록은 단방향
                drop_path=dpr[i],
                **factory_kwargs
            )
        ...
        if self.if_rope:
            # half_head_dim = dim // 2
            # BiFPN 피처맵 크기를 고려하여 적절한 grid_size 설정 필요
            # 여기서는 입력받은 img_size 기준으로 여유있게 생성
            self.rope = VisionRotaryEmbeddingFast(
                dim=dim,
                # pt_seq_len=img_size_h, 
                # ft_seq_len=img_size_w 
            )

        # --- 4. Final Normalization ---
        self.norm_f = nn.LayerNorm(dim, eps=1e-5, **factory_kwargs)
    ...
    def forward(self, x):
        """
        Input: (B, C, H, W) - BiFPN Feature Map
        ...
        # print(x.shape)
        # 1. Shape Transformation: (B, C, H, W) -> (B, L, C)
        # Vision Mamba는 (Batch, Length, Channel) 입력을 받습니다.
        x = x.flatten(2).transpose(1, 2)
        
        # ---------------------------------------------------------
        # [제공해주신 Vision Mamba Encoder Logic 그대로 적용]
        # ---------------------------------------------------------
        residual = None
        hidden_states = x
        ...
        if not self.if_bidirectional:
            ...
            for layer in self.layers:
                # RoPE
                if self.if_rope:
                    hidden_states = self.rope(hidden_states, H, W)
            ...
            # 양방향 처리 (Bidirectional - Vim Style)
            # 짝수 인덱스: 정방향 / 홀수 인덱스: 역방향
            for i in range(len(self.layers) // 2):
                if self.if_rope:
                    hidden_states = self.rope(hidden_states, H, W)
                    if residual is not None and self.if_rope_residual:
                        residual = self.rope(residual, H, W)
                ...
                # Backward Pass (Flip Input -> Block -> Flip Output)
                # 제공된 코드의 로직: hidden_states.flip([1])을 입력으로 줌
        ...
        # Final Normalization (Block 외부의 마지막 Norm)
        if not self.fused_add_norm:
            ...
            # Fused Add Norm 로직 (LayerNorm/RMSNorm)
        ...
        # 2. Shape Restoration: (B, L, C) -> (B, C, H, W)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        ...
        self.proj = nn.Conv2d(in_channels=64, out_channels=192, kernel_size=1)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def forward(self, x):
        # print('patchEmbed_x.shape')
        # print(x.shape)
        B, C, H, W = x.shape
...
def vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, stride=8, embed_dim=192, depth=1, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_divide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
